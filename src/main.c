...
int main(int argc, char* argv[]) {
    ...
    HF_CSV* my_csv = hf_csv_create(3, 3, TEST_ALLOCATOR);
    ...
    char* my_csv_string = hf_csv_to_string(my_csv, TEST_ALLOCATOR);
    printf("\n\nmy_csv_string pre resize:\n%s", my_csv_string);
    hf_csv_free_string(my_csv_string, TEST_ALLOCATOR);

    hf_csv_to_file(my_csv, "./result.csv", TEST_ALLOCATOR);

    hf_csv_resize(my_csv, 2, 2);
    my_csv_string = hf_csv_to_string(my_csv, TEST_ALLOCATOR);
    ...
    HF_CSV* loc_csv = hf_csv_create_from_file("./res/loc.csv", TEST_ALLOCATOR);
    ...
    hf_csv_to_file(loc_csv, "./lang_result.csv", TEST_ALLOCATOR);
    hf_csv_destroy(loc_csv);

    {//optional line feed test
        HF_CSV* line_feed = hf_csv_create_from_file("./res/line_feed.csv", TEST_ALLOCATOR);
        assert(line_feed);
        
        char* line_feed_str = hf_csv_to_string(line_feed, TEST_ALLOCATOR);
        printf("\n\nline_feed csv:\n%s\n", line_feed_str);
        hf_csv_free_string(line_feed_str, TEST_ALLOCATOR);

        hf_csv_destroy(line_feed);
    }
    {//badly formatted csv tests
        HF_CSV* bad_column_count = hf_csv_create_from_file("./res/bad_column_count.csv", TEST_ALLOCATOR);
        assert(!bad_column_count);
        hf_csv_destroy(bad_column_count);

        HF_CSV* bad_quote = hf_csv_create_from_file("./res/bad_quote.csv", TEST_ALLOCATOR);
        assert(!bad_quote);
        hf_csv_destroy(bad_quote);
    }
    {//carriage return
        const char* carriage_src = "a,b,c\r\nd,e,f\r\ng,h,i";
        HF_CSV* carriage = hf_csv_create_from_string(carriage_src, TEST_ALLOCATOR);
        assert(carriage);

        char* carriage_str = hf_csv_to_string(carriage, TEST_ALLOCATOR);
        assert(carriage_str);

        assert(strlen(carriage_src) == strlen(carriage_str));
        hf_csv_free_string(carriage_str, TEST_ALLOCATOR);

        hf_csv_destroy(carriage);
    }
    {//non carriage
        const char* line_feed_src = "a,b,c\nd,e,f\ng,h,i";
        HF_CSV* line_feed_csv = hf_csv_create_from_string(line_feed_src, TEST_ALLOCATOR);
